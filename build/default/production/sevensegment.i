...
# 139
PR6=255;
T6CON|=(uint8_t)(0x03<<0x03);
T6CON|=(uint8_t)(15<<0x03);
...
volatile uint8_t DigitValues[2+2];
volatile uint8_t DigitCommonPinsOffState;
volatile uint8_t CommonPinPositions[2+2];
...
SevenSegment.TotalDigits=2+2;
for(uint8_t i=4;i<(4+SevenSegment.TotalDigits);i++){
SevenSegment.DigitCommonPinsOffState|=(uint8_t)(1<<i);
SevenSegment.CommonPinPositions[i-4]=(uint8_t)(1<<i);
if((i-4)<2){
SevenSegment.DigitValues[i-4]=10;
}else{
SevenSegment.DigitValues[i-4]=18;
}
}
SevenSegment.CommonPinDriveMethod=(1<<1)|(1<<0);
...
TRISC&=(uint8_t)(~0xFF);
TRISB&=(uint8_t)(~SevenSegment.DigitCommonPinsOffState);
...
LATC=Segment_Val[SevenSegment.DigitValues[SevenSegment.CurrentlyActiveDigit]];
...
LATB|=(uint8_t)SevenSegment.DigitCommonPinsOffState;
...
if(SevenSegment.CurrentlyActiveDigit<2){
LATB&=(uint8_t)(~SevenSegment.CommonPinPositions[SevenSegment.CurrentlyActiveDigit]);
}else if(SevenSegment.CurrentlyActiveDigit==2){
...
if(Check_Indicator_LED_Status(4)){
Indicator_LED(4, 0);
}else{
Indicator_LED(4, 1);
}
